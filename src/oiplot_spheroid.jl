...
function draw_graticules(ax, star; nlat=5, nlon=8, color="black", linewidth=0.8, alpha=0.5,
    ...
    # Geometry is shared with the Makie layer — see graticule_segments in
    # src/oiplot_spheroid_core.jl. Everything below this line only draws.
    graticule_lines = graticule_segments(star; nlat=nlat, nlon=nlon,
        offset_west=offset_west, offset_north=offset_north, inclination=inclination,
        position_angle=position_angle, npoints=npoints, star_params=star_params)
